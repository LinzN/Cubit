...
public class FireFlag implements IFlags {
    ...
    public boolean getStatus(CubitLand cubitLand) {
        return cubitLand.getWGRegion().getFlag(DefaultFlag.FIRE_SPREAD) == StateFlag.State.ALLOW;
    }
    ...
    public String getStatusString(CubitLand cubitLand) {
        String statusString = CubitBukkitPlugin.inst().getYamlManager().getLanguage().fireOff;
        if (getStatus(cubitLand)) {
            statusString = CubitBukkitPlugin.inst().getYamlManager().getLanguage().fireOn;
        }
        return statusString;
    }
